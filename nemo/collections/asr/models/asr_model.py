...
class ASRModel(ModelPT, WithOptionalCudaGraphs, ABC):
    def multi_validation_epoch_end(self, outputs, dataloader_idx: int = 0):
        ...
        if outputs and 'val_loss' in outputs[0]:
            val_loss_mean = torch.stack([x['val_loss'] for x in outputs]).mean()
            val_loss = {'val_loss': val_loss_mean}

            tensorboard_logs.update(val_loss)

        if outputs and "val_wer_num" in outputs[0]:
        ...
        if outputs and "val_bleu_num" in outputs[0]:
    ...
    def multi_test_epoch_end(self, outputs, dataloader_idx: int = 0):
        ...
        if outputs and 'test_loss' in outputs[0]:
            val_loss_mean = torch.stack([x['test_loss'] for x in outputs]).mean()
            val_loss = {'test_loss': val_loss_mean}

            tensorboard_logs.update(val_loss)

        if outputs and "test_wer_num" in outputs[0]:
        ...
        if outputs and "test_bleu_num" in outputs[0]:
